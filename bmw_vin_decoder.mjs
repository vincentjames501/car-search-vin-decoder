...
export const getCodes2 = async (vin) => {
  ...
  return codeRows
    ? codeRows.map(row => row.match(specificRegex)[1])
    : [];
};
...
export const getCodes = async (vin) => {
  const last7OfVIN = vin.substring(vin.length - 7, vin.length).toLowerCase();
  const cacheFilePath = `./cache/bvzine_${vin}.json`;
  let payload;
  if (fs.existsSync(cacheFilePath)) {
    payload = JSON.parse(fs.readFileSync(cacheFilePath));
  } else {
    ...
    if (response.ok) {
      ...
      if (payload.myerror) {
        console.error(`Decoding failed for vin ${vin}`, payload);
        return [];
      }
      fs.writeFileSync(cacheFilePath, JSON.stringify(payload));
      await sleep(1000);
    } else {
      console.error(`Decoding failed for vin ${vin}`, response);
      return [];
    }
  }
